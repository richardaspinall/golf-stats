...
export function useVirtualCaddyController({
  ...
  const getCompletedShotActualDistanceMeters = (trail: PlannerShot[], shot: PlannerShot, index: number) => {
    ...
    return getRecordedDistanceFromFollowingShot(shot, followingShot);
  };

  const buildTrackedClubActualShots = (trail: PlannerShot[]) =>
    trail
      .map((shot, index) => {
        ...
        const actualDistanceMeters = getCompletedShotActualDistanceMeters(trail, shot, index);
      ...
      .filter((shot): shot is { shotId: number; club: string; actualMeters: number } => shot !== null);

  const deleteStaleTrackedClubActuals = async () => {
  ...
  return {
    ...
    getTrailRecordedDistanceMeters: (shot: PlannerShot, index: number) =>
      getTrailRecordedDistanceMeters(
        ...
        state.previousShotUseFlagAdjustment,
      ),
    getTrailSummary,
    actions: {
      saveShot,
